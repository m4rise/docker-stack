...
show_services() {
    ...
    echo "  - PostgreSQL: postgres:5432 (postgres/root)"
    echo "  - MariaDB: mariadb:3306 (root/root)"
    echo "  - MongoDB: mongo:27017 (root/root)"
    echo "  - Redis: redis:6379"
    echo "  - Elasticsearch: elasticsearch:9200"
    echo ""
    echo -e "${BLUE}Outils de développement:${NC}"
    echo "  - PGAdmin: https://pgadmin.localhost (admin@example.com/admin)"
    echo "  - RabbitMQ Management: https://rabbitmq.localhost (admin/admin)"
    echo "  - Inbucket (Email): https://inbucket.localhost"
    echo "  - Code Server: https://code.localhost"
    echo "  - Swagger UI: https://swagger.localhost"
    echo "  - MockServer: https://mock.localhost"
    echo ""
    echo -e "${BLUE}Monitoring:${NC}"
    echo "  - Grafana: https://grafana.localhost (admin/admin)"
    echo "  - Prometheus: https://prometheus.localhost (admin/admin)"
    echo ""
    echo -e "${BLUE}Utilitaires:${NC}"
    echo "  - HTTPBin: https://httpbin.localhost"
    echo ""
    echo -e "${YELLOW}Note: Les services avec (admin/admin) nécessitent une authentification${NC}"
}
